...
#' @return NULL
#' @noRd
.highmoi_run_gRNA_precomputation_at_scale <- function(pod_id, gRNA_precomp_dir, combined_perturbation_matrix, covariate_matrix, gene_matrix, log_dir, B, seed) {
...
#' Implements sceptre orig
#'
#' @inherit abstract_interface
#'
#' @export
sceptre_orig <- function(response_odm, grna_odm, response_grna_group_pairs) {
  # load the gene and grna matrix into memory
  gene_matrix <- load_whole_odm(response_odm)
  perturbation_matrix <- as.matrix(load_whole_odm(grna_odm))

  # get the grna feature covariates
  ...
  covariate_mat <- model.matrix(formula_object, covariate_data_frame)
  covariate_mat <- covariate_mat[,colnames(covariate_mat) != "(Intercept)"] |>
    as.data.frame()

  # compute perturbation matrix
  combined_perturbation_matrix <- combine_perturbations(perturbation_matrix = perturbation_matrix,
                                                        gRNA_groups_table = grna_group_data_frame)
  response_grna_group_pairs <- response_grna_group_pairs |>
    dplyr::rename("gene_id" = "response_id", "gRNA_group" = "grna_group")

  res <- run_sceptre_highmoi(gene_matrix = gene_matrix,
                             combined_perturbation_matrix = combined_perturbation_matrix,
                             covariate_matrix = covariate_mat,
                             gene_gRNA_group_pairs = response_grna_group_pairs,
                             parallel = FALSE)

  res <- res |> dplyr::rename("grna_group" = "gRNA_id", "response_id" = "gene_id")
  return(res)
}
